            ...
            {program.modules.map((mod) => (
              <div
                ...
                className={`rounded-xl border p-5 backdrop-blur-sm ${
                  isDark
                    ? `border-white/10 bg-white/[0.03] ${mod.locked ? 'opacity-60' : ''}`
                    : `border-rose-100 bg-white/40 ${mod.locked ? 'opacity-60' : ''}`
                }`}
              >
                <div className='flex items-center justify-between mb-1.5'>
                  ...
                  {mod.locked && (
                    <span
                      className={`text-xs ${isDark ? 'text-white/40' : 'text-ink-300'}`}
                    >
