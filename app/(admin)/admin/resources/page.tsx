...
const TYPE_CONFIG: Record<string, { icon: any; color: string; label: string }> =
  {
    pdf: { icon: FileIcon, color: 'bg-red-50 text-red-500', label: 'PDF' },
    audio: {
      icon: AudioIcon,
...
export default function ResourcesPage() {
  ...
  const tabs = [
    ...
    {
      id: '4-week',
      label: `4-Week (${resources.filter((r) => r.programId === '4-week').length})`,
    },
    {
      id: '8-week',
    ...
    <div className='space-y-6'>
          ...
          <button
            ...
            className={[
              ...
              filter === tab.id
                ...
                : 'bg-bg-surface text-ink-400 border-ink-100 hover:border-rose-300',
            ].join(' ')}
          >
            {tab.label}
          </button>
      ...
      {filtered.length === 0 ? (
        <div className='card flex flex-col items-center justify-center py-16 gap-3'>
          <FolderIcon size={40} className='text-rose-100' />
                      ...
                      <td className='px-5 py-4'>
                        <div className='flex items-center gap-2'>
                          <a
                            ...
                            className={`${ACTION_BTN} hover:border-blue-300 hover:text-blue-500`}
                          >
                            <GlobeIcon size={13} />
                          </a>
                          <Link href={`/admin/resources/${r.id}/edit`}>
                            <button
                              className={`${ACTION_BTN} hover:border-rose-300 hover:text-rose-400`}
                            >
                              <PenIcon size={13} />
                            </button>
                          </Link>
                          <button
                            ...
                            className={`${ACTION_BTN} hover:border-red-300 hover:text-red-500 disabled:opacity-40`}
                          >
                            {deleting === r.id ? (
                              <svg
