...
const PROGRAM_NAMES: Record<string, string> = {
  '4-week': 'Soul Blueprint Intensive',
...
export default function DashboardHome(): JSX.Element {
  ...
  return (
    <div className='space-y-8 stagger-children'>
          ...
          {enrollments.length === 0 ? (
            <div className='flex flex-col items-center justify-center py-10 gap-3 text-center'>
              <div className='w-12 h-12 rounded-full bg-rose-50 flex items-center justify-center'>
                <BookIcon size={20} className='text-rose-400' />
              </div>
                  ...
                  <Link key={e.id} href={`/dashboard/programs/${e.programId}`}>
                    <div
                      className='p-4 bg-rose-50/60 border border-rose-100 rounded-xl
                                   hover:border-rose-300 hover:-translate-y-0.5
            ...
            <ul className='space-y-3'>
              {sessions.map((s) => {
                ...
                return (
                  ...
                  >
                    <div
                      className='w-11 h-11 rounded-xl bg-bg-surface border border-rose-100
      ...
      {/* Daily affirmation */}
      <div className='card bg-burgundy-700 border-burgundy-700 relative overflow-hidden'>
        <div
          className='pointer-events-none absolute -top-10 -right-10 w-40 h-40
                        rounded-full bg-rose-400 opacity-10 blur-[60px]'
        />
        <div className='relative z-10 flex items-start gap-4'>
          <div
            className='w-10 h-10 rounded-full bg-rose-400/15 flex items-center
                          justify-center shrink-0 animate-[pulseRing_2.5s_ease-in-out_infinite]'
          >
            <StarIcon size={18} className='text-rose-300' />
          </div>
          <div>
            <p className='text-[11px] font-semibold uppercase tracking-widest text-rose-300 mb-2'>
              Today&apos;s Affirmation
            </p>
